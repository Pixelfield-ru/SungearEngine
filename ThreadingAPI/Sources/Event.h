...
namespace SGCore
{
    ...
    public:
        ...
        EventImpl& operator+=(HolderT* holder)
        {
            ...
            holder->m_unsubscribeFunc = [holder]()
            {
                ...
                {
                    listeningEvent->m_listeners.remove(std::make_pair(holder->m_hash, holder));
        ...
        {
            auto it = std::find_if(m_listeners.begin(), m_listeners.end(), [&funcHash](const std::pair<long, HolderT*>& a) {
                return a.second->m_hash == funcHash;
            });
        ...
        bool contains(const std::unique_ptr<HolderT>& eventHolder)
        {
            return contains(eventHolder->m_hash);
        }
        
        bool contains(const HolderT* eventHolder)
        {
            return contains(eventHolder->m_hash);
        }
    ...
    private:
        ...
        {
            m_listeners.sort([](const std::pair<long, HolderT*>& a, const std::pair<long, HolderT*>& b) {
