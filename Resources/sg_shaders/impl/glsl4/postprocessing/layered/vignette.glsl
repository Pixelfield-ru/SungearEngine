...
uniform sampler2D SG_VIGNETTE_final;
...
void main()
{
    ...
    if(SGPP_CurrentFXSubPassSeqIndex == 0)
    {
        ...
        float vignette = vignette(finalUV, SG_VIGNETTE_radius, SG_VIGNETTE_smoothness);

        fragColor = vec4(albedo.rgb * vignette, albedo.a);
    }
    // vignette write pass.
    // writes in SGPP_LayersColorsFX
    // reads SG_VIGNETTE_final
    else if(SGPP_CurrentFXSubPassSeqIndex == 1)
    {
        vec4 albedo = texture(SG_VIGNETTE_final, finalUV);
        fragColor = albedo;
    }
}
