...
SGSubPass(InstancingPass)
{
    SGSubShader(Vertex)
    {
        ...
        void main()
        {
            ...
            instanceModelMatrix[0] = texelFetch(u_matricesTextureBuffer, int(instanceID) * 4);
            instanceModelMatrix[1] = texelFetch(u_matricesTextureBuffer, int(instanceID) * 4 + 1);
            instanceModelMatrix[2] = texelFetch(u_matricesTextureBuffer, int(instanceID) * 4 + 2);
            instanceModelMatrix[3] = texelFetch(u_matricesTextureBuffer, int(instanceID) * 4 + 3);
