...
SGCore::PostProcessLayer& SGCore::ICamera::addPostProcessLayer(const std::string& ppLayerName,
...
void SGCore::ICamera::setPostProcessLayerShader(const Ref<Layer>& layer,
...
SGCore::PostProcessLayer& SGCore::ICamera::getPostProcessLayer(const Ref<Layer>& layer) noexcept
{
    const auto& foundPPLayer = m_postProcessLayers.find(layer);
...
void SGCore::ICamera::bindPostProcessFrameBuffer
...
void SGCore::ICamera::unbindPostProcessFrameBuffer() const noexcept
{
...
void SGCore::ICamera::clearPostProcessFrameBuffers() const noexcept
{
    for(const auto& ppLayer : m_postProcessLayers)
    {
...
SGCore::PostProcessLayer& SGCore::ICamera::getDefaultPostProcessLayer() noexcept
{
    // It is assumed that the technical layer always exists
    return m_postProcessLayers.find(m_technicalLayer)->second;
}

void SGCore::ICamera::updateLayersFrameBuffersMarkup() noexcept
{
