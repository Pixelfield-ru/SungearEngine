...
namespace SGCore::UI::Deserialization
{
    ...
    concept ImplDeserializable = requires(Base& base, UISourceTreeViewValue value) {
        requires SGCore::UI::ImplUISourceTreeViewValue<UISourceTreeViewValue>;
        { Base::template deserializeInto<UISourceTreeViewValue>(value, base) } -> std::convertible_to<DeserializeIntoResultType>;
    };
    ...
    static SGCore::UI::Deserialization::DeserializeIntoResultType deserializeInto(UISourceTreeViewValue value, type& field)

    template<typename UISourceTreeViewValue, typename Container>
    concept ImplDeserializableContainer = requires(Container& container, UISourceTreeViewValue value) {
        requires SGCore::UI::ImplUISourceTreeViewValue<UISourceTreeViewValue>;
        requires IsValidContainer<Container>::value;
        { IsValidContainer<Container>::ValueType_t::template deserializeIntoContainer<UISourceTreeViewValue, Container>(value, container) } -> std::convertible_to<DeserializeIntoResultType>;
    };


    // Check if Container is convertible to Container of that value type
    ...
    template<typename UISourceTreeViewValue, typename Container> requires \
        ...
        \
    static SGCore::UI::Deserialization::DeserializeIntoResultType deserializeIntoContainer(UISourceTreeViewValue value, Container& field)
}
