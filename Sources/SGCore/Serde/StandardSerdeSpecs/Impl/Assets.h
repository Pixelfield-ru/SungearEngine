...
namespace SGCore::Serde
{
    ...
    {
        auto assetAlias = valueView.container().template getMember<std::string>("m_alias");
        if(assetAlias)
        {
            valueView.m_data->m_alias = std::move(*assetAlias);
        }

        auto assetPath = valueView.container().template getMember<InterpolatedPath>("m_path");
        if(assetPath)
        {
    ...
    void SerdeSpec<ITexture2D, TFormatType>::serialize(SerializableValueView<const ITexture2D, TFormatType>& valueView, AssetsPackage& assetsPackage) noexcept
    {
        ...
        valueView.container().addMember("m_dataSizeInBytes", textureDataMarkup.m_sizeInBytes);

        /// We simply add lightweight data directly to the JSON file.
        valueView.container().addMember("m_width", valueView.m_data->m_width);
        valueView.container().addMember("m_height", valueView.m_data->m_height);
        valueView.container().addMember("m_channelsCount", valueView.m_data->m_channelsCount);
        valueView.container().addMember("m_internalFormat", valueView.m_data->m_internalFormat);
        valueView.container().addMember("m_format", valueView.m_data->m_format);
    }
    ...
    void SerdeSpec<ITexture2D, TFormatType>::deserialize(DeserializableValueView<ITexture2D, TFormatType>& valueView, AssetsPackage& assetsPackage) noexcept
    {
        ...
        auto channelsCount = valueView.container().template getMember<int>("m_channelsCount");
        auto internalFormat = valueView.container().template getMember<SGGColorInternalFormat>("m_internalFormat");
        auto format = valueView.container().template getMember<SGGColorFormat>("m_format");
    ...
    void SerdeSpec<AudioTrackAsset, TFormatType>::serialize(SerializableValueView<const AudioTrackAsset, TFormatType>& valueView) noexcept
    {
        ...
        valueView.container().addMember("m_sampleRate", valueView.m_data->m_sampleRate);
        valueView.container().addMember("m_byteRate", valueView.m_data->m_byteRate);
        valueView.container().addMember("m_blockAlign", valueView.m_data->m_blockAlign);
        valueView.container().addMember("m_bitsPerSample", valueView.m_data->m_bitsPerSample);
        valueView.container().addMember("m_frequency", valueView.m_data->m_frequency);
    }
    ...
    void SerdeSpec<AudioTrackAsset, TFormatType>::deserialize(DeserializableValueView<AudioTrackAsset, TFormatType>& valueView) noexcept
    {
        ...
        const auto audioFormat = valueView.container().template getMember<std::int16_t>("m_audioFormat");
        if(audioFormat)
        {
            valueView.m_data->m_audioFormat = *audioFormat;
        }

        const auto numChannels = valueView.container().template getMember<std::int32_t>("m_numChannels");
        if(numChannels)
        {
            valueView.m_data->m_numChannels = *numChannels;
        }

        const auto sampleRate = valueView.container().template getMember<std::int32_t>("m_sampleRate");
        if(sampleRate)
        {
            valueView.m_data->m_sampleRate = *sampleRate;
        }

        const auto byteRate = valueView.container().template getMember<std::uint32_t>("m_byteRate");
        if(byteRate)
        {
            valueView.m_data->m_byteRate = *byteRate;
        }

        const auto blockAlign = valueView.container().template getMember<std::uint16_t>("m_blockAlign");
        if(blockAlign)
        {
            valueView.m_data->m_blockAlign = *blockAlign;
        }

        const auto bitsPerSample = valueView.container().template getMember<std::uint16_t>("m_bitsPerSample");
        if(bitsPerSample)
        {
            valueView.m_data->m_bitsPerSample = *bitsPerSample;
        }

        const auto frequency = valueView.container().template getMember<std::uint32_t>("m_frequency");
        if(frequency)
        {
    ...
    {
        valueView.container().addMember("m_parts", valueView.m_data->m_parts, assetsPackage);
    }
    ...
    {
        auto parts = valueView.container().template getMember<decltype(valueView.m_data->m_parts)>("m_parts", assetsPackage);
        if(parts)
        {
    ...
    {
        // valueView.container().addMember("m_fileAssetPath", valueView.m_data->getFile()->getPath());
        valueView.container().addMember("m_shaderAnalyzedFile", valueView.m_data->m_shaderAnalyzedFile.lock());
        valueView.container().addMember("m_fileAsset", valueView.m_data->m_fileAsset.lock());
        valueView.container().addMember("m_autoRecompile", valueView.m_data->m_autoRecompile);
        valueView.container().addMember("m_textureBindings", valueView.m_data->m_textureBindings);
        valueView.container().addMember("m_defines", valueView.m_data->m_defines);
    }

    template<FormatType TFormatType>
    void SerdeSpec<IShader, TFormatType>::deserialize(DeserializableValueView<IShader, TFormatType>& valueView) noexcept
    {
        /*const auto m_fileAssetPath = valueView.container().template getMember<InterpolatedPath>("m_fileAssetPath");
        ...
        const auto m_shaderAnalyzedFile = valueView.container().template getMember<AssetRef<ShaderAnalyzedFile>>("m_shaderAnalyzedFile");
        if(m_shaderAnalyzedFile)
        {
            valueView.m_data->m_shaderAnalyzedFile = *m_shaderAnalyzedFile;
        }

        const auto m_fileAsset = valueView.container().template getMember<AssetRef<TextFileAsset>>("m_fileAsset");
        if(m_fileAsset)
        {
            valueView.m_data->m_fileAsset = *m_fileAsset;
        }

        const auto m_autoRecompile = valueView.container().template getMember<bool>("m_autoRecompile");
        if(m_autoRecompile)
        {
            valueView.m_data->m_autoRecompile = *m_autoRecompile;
        }

        auto m_textureBindings = valueView.container().template getMember<std::vector<ShaderTextureBinding>>("m_textureBindings");
        if(m_textureBindings)
        {
            valueView.m_data->m_textureBindings = std::move(*m_textureBindings);
        }

        auto m_defines = valueView.container().template getMember<decltype(IShader::m_defines)>("m_defines");
        if(m_defines)
        {
    ...
    {
        valueView.container().addMember("m_subPassName", valueView.m_data->m_subPassName);
        valueView.container().addMember("m_attributes", valueView.m_data->m_attributes);
        valueView.container().addMember("m_subShaders", valueView.m_data->m_subShaders, assetsPackage);
    }
    ...
    {
        auto m_subPassName = valueView.container().template getMember<std::string>("m_subPassName");
        if(m_subPassName)
        {
            valueView.m_data->m_subPassName = std::move(*m_subPassName);
        }

        auto m_attributes = valueView.container().template getMember<decltype(ShaderAnalyzedFile::m_attributes)>("m_attributes");
        if(m_attributes)
        {
            valueView.m_data->m_attributes = std::move(*m_attributes);
        }

        auto m_subShaders = valueView.container().template getMember<decltype(ShaderAnalyzedFile::m_subShaders)>("m_subShaders", assetsPackage);
        if(m_subShaders)
        {
    ...
    {
        const auto typeOpt = valueView.container().template getMember<SGSLESubShaderType>("m_type");
        auto dataOffsetOpt = valueView.container().template getMember<std::streamsize>("m_dataOffset");
        auto dataSizeInBytesOpt = valueView.container().template getMember<std::streamsize>("m_dataSizeInBytes");
    ...
    void SerdeSpec<IMaterial, TFormatType>::serialize(SerializableValueView<const IMaterial, TFormatType>& valueView, SharedDataT&&... sharedData) noexcept
    {
        ...
        valueView.container().addMember("m_transparencyType", valueView.m_data->m_transparencyType);
        valueView.container().addMember("m_shaders", valueView.m_data->m_shaders, std::forward<SharedDataT>(sharedData)...);
        valueView.container().addMember("m_meshRenderState", valueView.m_data->m_meshRenderState);

        valueView.container().addMember("m_textures", valueView.m_data->m_textures, std::forward<SharedDataT>(sharedData)...);

        valueView.container().addMember("m_diffuseColor", valueView.m_data->m_diffuseColor);
        valueView.container().addMember("m_specularColor", valueView.m_data->m_specularColor);
        valueView.container().addMember("m_ambientColor", valueView.m_data->m_ambientColor);
        valueView.container().addMember("m_emissionColor", valueView.m_data->m_emissionColor);
        valueView.container().addMember("m_transparentColor", valueView.m_data->m_transparentColor);
        valueView.container().addMember("m_shininess", valueView.m_data->m_shininess);
        valueView.container().addMember("m_metallicFactor", valueView.m_data->m_metallicFactor);
        valueView.container().addMember("m_roughnessFactor", valueView.m_data->m_roughnessFactor);
    }
    ...
    void SerdeSpec<IMaterial, TFormatType>::deserialize(DeserializableValueView<IMaterial, TFormatType>& valueView, SharedDataT&&... sharedData) noexcept
    {
        ...
        auto transparencyType = valueView.container().template getMember<MaterialTransparencyType>("m_transparencyType");
        if(transparencyType)
        {
            valueView.m_data->m_transparencyType = std::move(*transparencyType);
        }

        auto shaders = valueView.container().template getMember<std::unordered_map<std::string, AssetRef<IShader>>>("m_shaders", std::forward<SharedDataT>(sharedData)...);
        if(shaders)
        {
            valueView.m_data->m_shaders = std::move(*shaders);
        }

        const auto meshRenderState = valueView.container().template getMember<MeshRenderState>("m_meshRenderState");
        if(meshRenderState)
        {
            valueView.m_data->m_meshRenderState = *meshRenderState;
        }

        auto textures = valueView.container().template getMember<decltype(valueView.m_data->m_textures)>("m_textures", std::forward<SharedDataT>(sharedData)...);
        if(textures)
        {
            valueView.m_data->m_textures = std::move(*textures);
        }

        const auto diffuseColor = valueView.container().template getMember<glm::vec4>("m_diffuseColor");
        if(diffuseColor)
        {
            valueView.m_data->m_diffuseColor = *diffuseColor;
        }

        const auto specularColor = valueView.container().template getMember<glm::vec4>("m_specularColor");
        if(specularColor)
        {
            valueView.m_data->m_specularColor = *specularColor;
        }

        const auto ambientColor = valueView.container().template getMember<glm::vec4>("m_ambientColor");
        if(ambientColor)
        {
            valueView.m_data->m_ambientColor = *ambientColor;
        }

        const auto emissionColor = valueView.container().template getMember<glm::vec4>("m_emissionColor");
        if(emissionColor)
        {
            valueView.m_data->m_emissionColor = *emissionColor;
        }

        const auto transparentColor = valueView.container().template getMember<glm::vec4>("m_transparentColor");
        if(transparentColor)
        {
            valueView.m_data->m_transparentColor = *transparentColor;
        }

        const auto shininess = valueView.container().template getMember<float>("m_shininess");
        if(shininess)
        {
            valueView.m_data->m_shininess = *shininess;
        }

        const auto metallicFactor = valueView.container().template getMember<float>("m_metallicFactor");
        if(metallicFactor)
        {
            valueView.m_data->m_metallicFactor = *metallicFactor;
        }

        const auto roughnessFactor = valueView.container().template getMember<float>("m_roughnessFactor");
        if(roughnessFactor)
        {
    ...
    void SerdeSpec<IMeshData, TFormatType>::serialize(SerializableValueView<const IMeshData, TFormatType>& valueView, AssetsPackage& assetsPackage) noexcept
    {
...

        {
            ...
            valueView.container().addMember("m_indicesOffset", dataMarkup.m_offset);
            valueView.container().addMember("m_indicesSizeInBytes", dataMarkup.m_sizeInBytes);
        }

        {
            AssetsPackage::DataMarkup dataMarkup = assetsPackage.addData(valueView.m_data->m_vertices);

            valueView.container().addMember("m_verticesOffset", dataMarkup.m_offset);
            valueView.container().addMember("m_verticesSizeInBytes", dataMarkup.m_sizeInBytes);
        }
        ...
        valueView.container().addMember("m_generatePhysicalMesh", valueView.m_data->m_physicalMesh != nullptr);

        valueView.container().addMember("m_material", valueView.m_data->m_material, assetsPackage);
    }
    ...
    {
        auto aabb = valueView.container().template getMember<AABB<>>("m_aabb");
        if(aabb)
        {
        ...
        {
            auto dataOffsetOpt = valueView.container().template getMember<std::streamsize>(
                    "m_indicesOffset"
            );
            auto dataSizeInBytesOpt = valueView.container().template getMember<std::streamsize>(
        ...
        auto generatePhysicalMesh = valueView.container().template getMember<bool>("m_generatePhysicalMesh");
        if(generatePhysicalMesh)
        {
            valueView.m_data->generatePhysicalMesh();
        }

        auto material = valueView.container().template getMember<AssetRef<IMaterial>>("m_material");
        if(material)
        {
    ...
    void SerdeSpec<ModelAsset, TFormatType>::deserialize(DeserializableValueView<ModelAsset, TFormatType>& valueView, AssetsPackage& assetsPackage) noexcept
    {
        ...
        auto modelName = valueView.container().template getMember<std::string>("m_modelName");
        if(modelName)
        {
            valueView.m_data->m_modelName = std::move(*modelName);
        }

        auto rootNode = valueView.container().template getMember<Ref<Node>>("m_rootNode", assetsPackage);
        if(rootNode)
        {
    ...
    {
        valueView.container().addMember("m_rootBone", valueView.m_data->m_rootBone, assetsPackage);
        valueView.container().addMember("m_allBones", valueView.m_data->m_allBones, assetsPackage);
    }
    ...
    {
        auto rootBone = valueView.container().template getMember<AssetRef<Bone>>("m_rootBone", assetsPackage);
        if(rootBone)
        {
            valueView.m_data->m_rootBone = std::move(*rootBone);
        }

        auto allBones = valueView.container().template getMember<decltype(Skeleton::m_allBones)>("m_allBones", assetsPackage);
        if(allBones)
        {
    ...
    {
        // todo: is it correct?
        valueView.container().addMember("m_id", valueView.m_data->m_id);
        valueView.container().addMember("m_boneName", valueView.m_data->m_boneName);
        valueView.container().addMember("m_offsetMatrix", valueView.m_data->m_offsetMatrix);
        valueView.container().addMember("m_affectedMeshesBoneData", valueView.m_data->m_affectedMeshesBoneData, assetsPackage);
    ...
    {
        // todo: is it correct?
        auto id = valueView.container().template getMember<decltype(Bone::m_id)>("m_id");
        if(id)
        {
            valueView.m_data->m_id = std::move(*id);
        }

        auto boneName = valueView.container().template getMember<std::string>("m_boneName");
        if(boneName)
        {
            valueView.m_data->m_boneName = std::move(*boneName);
        }

        auto offsetMatrix = valueView.container().template getMember<glm::mat4>("m_offsetMatrix");
        if(offsetMatrix)
        {
            valueView.m_data->m_offsetMatrix = std::move(*offsetMatrix);
        }

        auto affectedMeshesBoneData = valueView.container().template getMember<decltype(Bone::m_affectedMeshesBoneData)>("m_affectedMeshesBoneData", assetsPackage);
        if(affectedMeshesBoneData)
        {
            valueView.m_data->m_affectedMeshesBoneData = std::move(*affectedMeshesBoneData);
        }

        auto children = valueView.container().template getMember<decltype(Bone::m_children)>("m_children", assetsPackage);
        if(children)
        {
    ...
    {
        valueView.container().addMember("m_affectedMesh", valueView.m_data->m_affectedMesh, assetsPackage);

        {
            AssetsPackage::DataMarkup dataMarkup = assetsPackage.addData(valueView.m_data->m_weights);

            valueView.container().addMember("m_weightsDataMarkupInPackage", dataMarkup);
        }
    }
    ...
    {
        auto affectedMesh = valueView.container().template getMember<AssetRef<IMeshData>>("m_affectedMesh", assetsPackage);
        if(affectedMesh)
        {
        ...
        {
            auto dataMarkupOpt = valueView.container().template getMember<AssetsPackage::DataMarkup>(
    ...
    {
        valueView.container().addMember("m_skeletalAnimations", valueView.m_data->m_skeletalAnimations, assetsPackage);
    }
    ...
    void SerdeSpec<AnimationsFile, TFormatType>::deserialize(DeserializableValueView<AnimationsFile, TFormatType>& valueView, AssetsPackage& assetsPackage) noexcept
    {
        ...
        auto skeletalAnimations = valueView.container().template getMember<decltype(AnimationsFile::m_skeletalAnimations)>("m_skeletalAnimations", assetsPackage);
    ...
    {
        valueView.container().addMember("m_animationName", valueView.m_data->m_animationName);
        valueView.container().addMember("m_duration", valueView.m_data->m_duration);
        valueView.container().addMember("m_ticksPerSecond", valueView.m_data->m_ticksPerSecond);
        valueView.container().addMember("m_bonesAnimations", valueView.m_data->m_bonesAnimations, assetsPackage);
    }
    ...
    {
        auto animationName = valueView.container().template getMember<std::string>("m_animationName");
        if(animationName)
        {
            valueView.m_data->m_animationName = std::move(*animationName);
        }

        auto duration = valueView.container().template getMember<float>("m_duration");
        if(duration)
        {
            valueView.m_data->m_duration = std::move(*duration);
        }

        auto ticksPerSecond = valueView.container().template getMember<float>("m_ticksPerSecond");
        if(ticksPerSecond)
        {
            valueView.m_data->m_ticksPerSecond = std::move(*ticksPerSecond);
        }

        auto bonesAnimations = valueView.container().template getMember<decltype(SkeletalAnimationAsset::m_bonesAnimations)>("m_bonesAnimations", assetsPackage);
        if(bonesAnimations)
        {
    ...
    void SerdeSpec<SkeletalBoneAnimation, TFormatType>::serialize(SerializableValueView<const SkeletalBoneAnimation, TFormatType>& valueView, AssetsPackage& assetsPackage) noexcept
    {
...

        {
            ...
            valueView.container().addMember("m_positionKeysMarkupInPackage", dataMarkup);
        }

        {
            AssetsPackage::DataMarkup dataMarkup = assetsPackage.addData(valueView.m_data->m_rotationKeys);

            valueView.container().addMember("m_rotationKeysMarkupInPackage", dataMarkup);
        }

        {
            AssetsPackage::DataMarkup dataMarkup = assetsPackage.addData(valueView.m_data->m_scaleKeys);

            valueView.container().addMember("m_scaleKeysMarkupInPackage", dataMarkup);
        }
    }
    ...
    void SerdeSpec<AssetRef<AssetT>, TFormatType>::deserialize(DeserializableValueView<AssetRef<AssetT>, TFormatType>& valueView, AssetsPackage&) noexcept
    {
        ...
        auto assetPath = valueView.container().template getMember<InterpolatedPath>("m_path");
        const auto assetTypeID = valueView.container().template getMember<size_t>("m_assetTypeID");
        auto assetAlias = valueView.container().template getMember<std::string>("m_alias");
        const auto assetStorageType = valueView.container().template getMember<AssetStorageType>("m_storedBy");
    ...
    {
        valueView.container().addMember("m_offset", valueView.m_data->m_offset);
        valueView.container().addMember("m_sizeInBytes", valueView.m_data->m_sizeInBytes);
    }
    ...
    {
        const auto m_offset = valueView.container().template getMember<std::streamsize>("m_offset");
        if(m_offset)
        {
            valueView.m_data->m_offset = *m_offset;
        }

        const auto m_sizeInBytes = valueView.container().template getMember<std::streamsize>("m_sizeInBytes");
        if(m_sizeInBytes)
        {
